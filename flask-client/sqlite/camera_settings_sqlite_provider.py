...
class CameraSettingsSQLiteProvider:
    ...
    def _init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            if cursor.fetchone()[0] == 0:
                cursor.execute('''
                    INSERT INTO camera_settings (name, min_conf, min_d_detect, min_d_save, particle_bb_dimension_factor, est_particle_volume_x, est_particle_volume_exp)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', ('default', 0.8, 200, 200, 0.9, 8.357470139e-11, 3.02511466443))
            conn.commit()
    ...
    def insert_settings(self, name: str, min_conf: float, min_d_detect: int, min_d_save: int,
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute('''
                INSERT INTO camera_settings (name, min_conf, min_d_detect, min_d_save, particle_bb_dimension_factor, est_particle_volume_x, est_particle_volume_exp)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ''', (name, min_conf, min_d_detect, min_d_save, particle_bb_dimension_factor, est_particle_volume_x, est_particle_volume_exp))
            conn.commit()
            return cursor.lastrowid

    def get_settings(self, name: str) -> Optional[Tuple[int, str, float, int, int, float, float, float, datetime, datetime]]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT id, name, min_conf, min_d_detect, min_d_save, particle_bb_dimension_factor, est_particle_volume_x, est_particle_volume_exp, created_at, updated_at
            ...
            if row:
                ...
                return (row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], created_at, updated_at)
            return None

    def list_settings(self) -> List[Tuple[int, str, float, int, int, float, float, float, str, str]]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute('''
                SELECT id, name, min_conf, min_d_detect, min_d_save, particle_bb_dimension_factor, est_particle_volume_x, est_particle_volume_exp, created_at, updated_at
    ...
    def get_default_settings(self) -> Optional[Tuple[int, str, float, int, int, float, float, float, datetime, datetime]]:
